...
const createProperty = async (userId, payload) => {
...
const getAllProperties = async (query) => {
    const { location, propertyType, minPrice, maxPrice, amenities } = query;
    const parsedMinPrice = minPrice !== undefined && minPrice !== ""
        ? Number(minPrice)
        : undefined;
    const parsedMaxPrice = maxPrice !== undefined && maxPrice !== ""
        ? Number(maxPrice)
        : undefined;
    if ((parsedMinPrice !== undefined &&
        ...
        throw new AppError(400, "Invalid price range");
    }
    const properties = await prisma.property.findMany({
        where: {
